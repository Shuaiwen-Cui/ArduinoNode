...
bool sync_check_rf_online()
{
    ...
    for (uint8_t id = 1; id <= NUM_NODES; ++id)
    {
        ...
        while (millis() - start_wait < RF_RESPONSE_WINDOW_MS && count < PONG_REPETITIONS)
        {
            ...
            // GATEWAY SIDE
            uint32_t t4 = millis(); // receive time after rf_receive()
                ...
                strncmp(response.payload, RF_PONG_PAYLOAD, 4) == 0)
            {
                ...
                if (parsed == 2 && t3 > t2)
                {
                    ...
                    int32_t offset = (delta_master - delta_node) / 2;

                    sum_drift += drift;
                    sum_offset += offset;
                    count++;

                    // === Debug Output (After computation) ===
                    Serial.print("   ↳ Sample ");
                    Serial.print(count);
                    Serial.print(": Drift = ");
                    ...
                    Serial.print("     t1 (Master Send): ");
                    Serial.println(t1);
                    Serial.print("     t2 (Node Recv)  : ");
                    Serial.println(t2);
                    Serial.print("     t3 (Node Send)  : ");
                    Serial.println(t3);
                    Serial.print("     t4 (Master Recv): ");
                    Serial.println(t4);
                }
            }
        }

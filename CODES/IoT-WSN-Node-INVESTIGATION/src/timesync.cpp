...
bool rf_time_sync()
{
    ...
    double drift_list[SYNC_ROUNDS] = {0.0}; // New: record drift values
    uint8_t received = 0;
    ...
    for (uint8_t i = 1; i < SYNC_ROUNDS; ++i)
    {
        ...
        drift_list[i] = drift_i;

        drift_sum += drift_i;
        drift_count++;

        if (drift_i > drift_max)
            drift_max = drift_i;
        if (drift_i < drift_min)
            drift_min = drift_i;
    }
    ...
    // === Step 4: Update drift_ratio and time mapping ===
    Time.drift_ratio = 1.0 + drift_avg;
    Time.time_offset = gateway_time[SYNC_ROUNDS - 1];
    Time.last_sync_running_time = local_time[SYNC_ROUNDS - 1];

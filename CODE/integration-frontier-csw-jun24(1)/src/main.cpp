...
void loop()
{
  if (node_status.get_state() == NodeState::IDLE || node_status.get_state() == NodeState::COMMUNICATING)
  {
    // routine operation
  ...
  else if (node_status.get_state() == NodeState::SAMPLING)
  {
    ...
    if (now_unix_ms > sensing_scheduled_end_ms)
    {
      ...
      node_status.set_state(NodeState::IDLE);
      node_status.node_flags.sensing_requested = false; // Reset sensing requested flag
      node_status.node_flags.sensing_scheduled = false; // Reset sensing scheduled flag
      rgbled_set_all(CRGB::White); // Set LED to white when idle
